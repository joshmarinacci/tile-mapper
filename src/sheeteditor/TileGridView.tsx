...
export function TileGridView(props: {
    ...
    const showPopup = (e: React.MouseEvent<HTMLElement>) => {
        ...
        setDown(false)
        if (tile) {
            pm.show_at(<TilePopupMenu value={tile} grid={model}
                                      sheet={options.sheet}/>, e.target, "left")
        }
    }
    ...
    return <canvas ref={ref} className={'draggable-grid-view'}
                   width={size.w * options.scale * 5} height={size.h * options.scale * 5}
